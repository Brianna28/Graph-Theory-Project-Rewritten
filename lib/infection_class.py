...
class infection_graph: 
    ...
    """    
    def __init__(self,network: np.ndarray, initial_infected: int, intial_immune: int,enable_vis: bool):
        ...
        self.vertices = [x for x in range(self.no_nodes)] #this is a list of all the vertices in the network
        self.edges = np.sum(self.graph)//2 #the number of edges in the network
        self.degrees = dict(zip(self.vertices,[i.sum() for i in self.graph])) #returns a dictionary with the nodes as keys and their degree as value
        histo_maker = lambda lst: {item: lst.count(item) for item in lst}
        self.histogram = histo_maker([i.sum() for i in self.graph]) #Creates a dictionary where the degree is the key and the frequency of that degree is the value
